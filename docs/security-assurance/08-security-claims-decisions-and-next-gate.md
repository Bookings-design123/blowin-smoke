...
|---|---|---|
| “End-to-end encrypted wholesale messaging” | `SAFE IF VERIFIED` | Only for designated message/approved-attachment content; a selected maintained implementation passes independent cryptographic/application review; keys remain at authorized endpoints; device membership and key changes are visible; no plaintext logs/backups/analytics; metadata and endpoint/code-delivery limits are disclosed. |
| “Blowin' Smoke cannot read your private wholesale messages” | `REQUIRES QUALIFICATION` | Safer: “Our messaging intermediary is designed not to hold the keys needed to decrypt designated wholesale messages. Authorized participants can read them on their devices.” Must disclose web-code delivery, endpoint, participant, reporting/declassification, and metadata boundaries. |
| “Even the owner cannot read your conversations” | `PROHIBITED` | An owner could be an authorized representative/endpoint or control browser code; the absolute also ignores deliberate customer reporting/declassification. |
...
| “Screenshots are blocked” | `PROHIBITED` | Browser/PWA prevention fails; native/managed controls are platform-limited best effort. |
| “Supported native clients use screen-capture protections” | `REQUIRES QUALIFICATION` | Name exact platform/version/control and whether it blocks, detects, or redacts. Disclose gaps, screenshots already taken, compromised devices, and cameras. |
| “Screen recording is impossible” | `PROHIBITED` | No assessed client class provides this universal guarantee. |
| “Private wholesale inventory cannot be copied” | `PROHIBITED` | Authorized viewers can transcribe, inspect requests, capture pixels, or use a camera. |
...
| “Wholesale room access is restricted to approved accounts” | `SAFE IF VERIFIED` | Every object/field/media request is server-authorized; no public enumeration/bulk leak; current grant/revocation/rate controls pass adversarial tests. |
...
| “The manifest is erased from your device after five minutes” | `PROHIBITED` | Browser memory, offline tabs, print, screenshots, cameras, and compromised devices cannot be guaranteed erased. |
...
| “Optional Onion access” | `SAFE IF VERIFIED` | A live controlled Onion address passes origin/resource/session/key continuity/parity tests and operations monitoring. Do not imply anonymity or universal client feature parity. |
...
### SEC-02-ADR-001 — Browser-first remains the default feasibility direction

**Decision:** Preserve browser access because it meets owner intent, accessibility, low friction, and Onion alignment.
**Qualification:** Browser code delivery is an active trust boundary; “owner can never read” is not safe.
**Consequence:** Dedicated origin, third-party exclusion, release integrity/transparency, endpoint verification, and whole-product review are mandatory.
...
**Decision:** PWA may be evaluated for convenience, not as a solution to code substitution, screenshot, recording, or key custody.
**Consequence:** No offline manifest/private media cache; update/cache risks need separate evidence.

### SEC-02-ADR-004 — Signed native is a future optional assurance tier

**Decision:** Platform signing and protected key facilities materially improve release/key options; capture remains best effort and customer/operations burden is high.
**Consequence:** Do not require native for ordinary retail or claim Onion compatibility absent separate audited transport.
...
### SEC-02-ADR-006 — Capture claims are deterrence/attribution only

**Decision:** Browser/PWA capture prevention and second-camera prevention fail. Supported native/managed controls are qualified best effort.
**Consequence:** Use least exposure, short leases, individualized visible marks, optional validated forensic marks, audit, appeal, and revoke. No prevention claim.
...
**Consequence:** Refresh/replay never extends clocks; open/offline/captured content cannot be guaranteed gone; canonical records remain. Accessibility and the remaining 25-minute reference model require owner decisions.

### SEC-02-ADR-009 — Onion is optional, isolated, and non-anonymous

**Decision:** An Onion entrance may front the same canonical application through an isolated gateway after origin/resource/session/key/operations tests.
**Consequence:** Separate public/Onion sessions and explicit reauthentication are the safest default; third-party browser resources are denied by default; account/transaction anonymity is never claimed.
...
No demonstrable technical contradiction requires rewriting SEC-01. SEC-02 adds these explicit feasibility qualifications:

| Topic | SEC-01/owner intent | Evidence result | Required treatment |
|---|---|---|---|
| Server-blind browser messaging | Required | Technically feasible, but origin can substitute client code | Preserve requirement; qualify owner/admin absolute claim; strengthen release boundary |
...
| Strong capture prevention | Desired | Browser/PWA and camera prevention fail | Reframe as prevention of unauthorized access plus deterrence/attribution of authorized leakage |
...
| Thirty-minute dispute | Required | Feasible, but 25-minute information gap after view expires | Owner selects minimal reference, early initiation, or staff assistance |
...
- Browser release authority, reproducible build/transparency monitor, dual-control emergency update, dependency and third-party exclusion.
...
- Native supported versions/control behavior and whether native remains optional.
...
- Minimal-reference/early-initiation/staff-assisted solution for the +5m to +30m gap; outage/grace and receipt policy.
...
Because the supplied SEC-02 task source ends mid-section before any surviving next-gate name, the following name is a **proposal, not recovered owner wording**:
...
2. **Client integrity:** dedicated origin, reproducible builds, release manifest/transparency monitor, dual-control delivery, dependency/SBOM/vulnerability process, malicious-origin threat exercise.
...
4. **Media/capture proof:** object authorization, leases, cache behavior, 4K segments/range/expiry, visible derivative, forensic candidate if any, platform capture matrix, second-camera disclosure.
5. **Manifest proof:** atomic first reveal, concurrent retry, +5m server deny, offline/frozen/back-forward/cache/memory/output tests, +30m idempotent dispute receipt, accessible timing and reference-surface decision.
6. **Onion proof:** protected topology, complete browser resource trace, origin/session leak tests, Tor security-level compatibility, outage/key compromise/address migration, public/Onion canonical parity.
...
- browser/native claims match tested behavior and malicious-code/endpoint residuals are disclosed;
...
- **BROWSER/PWA CAPTURE PREVENTION: FAIL**
- **NATIVE CAPTURE CONTROLS: CONDITIONAL / BEST-EFFORT**
...
- **THIRTY-MINUTE DISPUTE CLOCK: FEASIBLE; REFERENCE/OUTAGE POLICY OPEN**
