...
|---|---|---|---|---|
| Customer endpoint plaintext/view state | Authorized endpoint | Canonical linked order reaches approved delivered/terminal trigger and no hold/reopen applies | Remove from active UX; delete local indexed plaintext/key references best effort; record acknowledgement/failure without content | Screenshots, exports, OS remnants, compromised/offline endpoint |
...
| Export | Explicitly authorized endpoint/user | Export-specific purpose | No export by default; if ever approved, classify and warn | Recall after export |
